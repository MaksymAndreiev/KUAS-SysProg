...
struct Link {
    ...
    struct Link *prev;  // pointer to previous link, or 0 at start of list
};
 ...
 */
struct Link *newLink(ELEMENT_T data, struct Link *prev, struct Link *next) {
...
void List_addFirst(struct List *list, ELEMENT_T data) {
    struct Link *link = newLink(data, 0, list->first);
...
void List_addLast(struct List *list, ELEMENT_T data) {
    struct Link *link = newLink(data, list->last, 0);
    if (list->first == 0) {
...
void List_apply(struct List *list, void (*fn)(ELEMENT_T)){
    struct Link *link = list->first;
    while(link != 0){
        fn(link->data);
        link = link->next;
...
ELEMENT_T List_removeLast(struct List *list){
    if (list->first == 0) {
        printf("List is empty\n");
        exit(1);
    }
    struct Link *link = list->last;
    list->last = link->prev;
    if (list->last == 0) {
        list->first = 0;
    } else {
