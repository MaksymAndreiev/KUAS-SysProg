...
struct Assoc *Map_find(struct Map *map, char *key) {
    size_t i = stringHash(key) % map->size;
    size_t h = i;

    while (i < map->size) {
        struct Assoc *assoc = map->assocs[i];
        if (assoc && strcmp(assoc->key, key) == 0) {
            return assoc;
        } else if (!assoc) {
...
void Map_remove(struct Map *map, char *key) {
    ...
    if (map->assocs[h] && strcmp(map->assocs[h]->key, key) == 0) {
        free(map->assocs[h]->key);
        free(map->assocs[h]);
        map->assocs[h] = NULL;

        size_t i = (h + 1) % map->size;
        while (map->assocs[i] && stringHash(map->assocs[i]->key) % map->size == h) {
            map->assocs[h] = map->assocs[i];
            map->assocs[i] = NULL;
            h = i;
            i = (i + 1) % map->size;
        }
